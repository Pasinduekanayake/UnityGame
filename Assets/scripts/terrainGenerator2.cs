...
public class terrainGenerator2 : MonoBehaviour
{
    ...
    private void DrawVertice()
    {
        ...
        {
            Instantiate(verticesPrefab, verticesArray[k], Quaternion.identity, transform);
        }
    }
    ...
    {
        verticesArray = new Vector3[(width+1)*(depth+1)];
        int trackVertices = 0;
        for(int z = 0; z < (depth + 1); z++)
        {
            for(int x = 0; x < (width + 1); x++)
            {
                ...
                verticesArray[trackVertices] = new Vector3(x, y, z);
                trackVertices++;
            }
        }
    }
